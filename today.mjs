...
const writeState = () => writeFileSync(STATE, "// generated by today.mjs — snapshot of queue.json for drill.html when opened as a file\nwindow.STATE = " + JSON.stringify(db) + ";\n");
...
if (cmd === "serve") {
  ...
  const logHit = (p) => {
    const it = byId(Number(p.id)); if (!it) return false;
    const d = p.d || TODAY;
